...
import com.small.library.ejb.util.CodeEntity;
...
public abstract class PopulateListServlet extends RPCServlet
{
	/**
	...
	*/
	...
	/** Implement this function to retrieve required data and generate output script
			...
			@return a <I>Collection</I> of <I>CodeEntity</I> objects.
	*/
	protected abstract Collection getCodeEntities(HttpServletRequest request)
		...
		throws ServletException, IOException
	{
		try
		{
			Collection elements = getCodeEntities(request);
			...
			while (it.hasNext())
			{
				CodeEntity element = (CodeEntity)
					javax.rmi.PortableRemoteObject.narrow(it.next(), CodeEntity.class);
				...
				String value = element.getCodeValue();

				// Must escape any quotes.
				if (-1 < value.indexOf("\""))
					value = value.replace('\"', '\'');

				ids.append("\"" + element.getCodeId() + "\"");
				values.append("\"" + value + "\"");					
			}
