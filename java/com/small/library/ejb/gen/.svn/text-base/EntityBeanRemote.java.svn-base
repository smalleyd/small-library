...
/***************************************************************************************
*
*	Generates interface for EJB Entity Bean remote interfaces. The remote interfaces
...
public class EntityBeanRemote extends EntityBeanBase
{
	/******************************************************************************
	*
	*	Constants
	...
	*****************************************************************************/
	...
	/** Helper method - gets the full class/interface name of the remote
	...
	*/
	public EntityBeanRemote(PrintWriter pWriter,
	...
	*****************************************************************************/
	...
	private void writeHeader() throws IOException
	{
		...
		writeLine();
		writeLine("import javax.ejb.EJBObject;");
		writeLine("import javax.ejb.FinderException;");	// For imported key accessors.
		writeLine("import javax.naming.NamingException;");	// For imported key accessors.
		writeLine();
		writeLine("/**********************************************************************************");
		writeLine("*");
		writeLine("*\tEntity Bean remote interface that represents the " + getTable().getName());
		writeLine("*\ttable.");
		writeLine("*");
	...
	{
		writeLine();
		writeLine("public interface " + getObjectName() + CLASS_NAME_SUFFIX + " extends EJBObject");
		writeLine("{");
	}
	...
	private void writeAccessorMethods() throws IOException
	{
		...
		for (int i = 0; i < m_ColumnInfo.length; i++)
		{
			...
			writeLine("\t*/");
			writeLine("\tpublic " + m_ColumnInfo[i].javaType + " " + m_ColumnInfo[i].accessorMethodName + "() throws RemoteException;");
		}
		...
		writeLine("\t*/");
		writeLine("\tpublic " + valueObjectName + " getAll() throws RemoteException;");
	}
	...
	private void writeImportedKeysAccessorMethods() throws IOException
	{
		...
		for (int i = 0; i < m_ColumnInfo.length; i++)
		{
			...
			writeLine("\t\tthrows RemoteException;");
		}
	}
	...
	private void writeMutatorMethods() throws IOException	
	{
		...
		for (int i = 0; i < m_ColumnInfo.length; i++)
		{
			...
			writeLine("\t    \"" + m_ColumnInfo[i].columnName + "\" field.");
			writeLine("\t*/");
			writeLine("\tpublic void " + m_ColumnInfo[i].mutatorMethodName + "(" + m_ColumnInfo[i].javaType + " newValue) throws RemoteException;");
		}
		...
		writeLine("\t*/");
		writeLine("\tpublic void setAll(" + valueObjectName + " newValue) throws RemoteException;");
	}
	...
	*****************************************************************************/
	...
	/** Command line entry point.
		...
		@param strArg7 package name of the remote interfaces.
	*/
	public static void main(String strArgs[])
	{
		try
		{
			...
			EntityBeanRemote pGenerator =
				new EntityBeanRemote((PrintWriter) null, strAuthor,
		...
		catch (IllegalArgumentException pEx)
		{
			...
			System.out.println("Usage: java " + EntityBeanRemote.class.getName() + " Output directory");
